...
class Compute:
    ...
    def __init__(self,h,delta_factor,iter):
        ...
        for i in range(0,n):
            for j in range(0,n):
                ...
                if j*h < h and i * h > 15 - h :
                    self.fix.append(index)
                    #plt.scatter(i*h,j*h)

                if j * h > 15 - h  and i * h > 15 - h :
                    self.fix.append(index)
                    #plt.scatter(i*h,j*h)
        ...
        if self.iter == 1:
            ...
            filehandler = open("bond-based-2d-plate-point-"+str(self.h)+"-"+str(self.delta_factor)+"-"+str(self.iter-1)+"-displacement.npy", "rb")
    ...
    def plot(self,iter):
        ...
        plt.savefig("bond-based-2d-plate-point-u-x-"+str(self.h)+"-"+str(self.delta_factor)+"-"+str(step)+".pdf",bbox_inches='tight')
        plt.clf()
        # Plot u_y
        ...
        plt.xlabel("Position $x$")
        plt.ylabel("Position $y$")
        plt.savefig("bond-based-2d-plate-point-u-y-"+str(self.h)+"-"+str(self.delta_factor)+"-"+str(step)+".pdf",bbox_inches='tight')
        plt.clf()
        # Plot damage
        ...
        plt.xlabel("Position $x$")
        plt.ylabel("Position $y$")
        plt.savefig("bond-based-2d-plate-point-d-"+str(self.h)+"-"+str(self.delta_factor)+"-"+str(step)+".pdf",bbox_inches='tight')
        plt.clf()

    def dump(self,iter):
        step = iter + self.iter
        filehandler = open("bond-based-2d-plate-point-"+str(self.h)+"-"+str(self.delta_factor)+"-"+str(step)+"-displacement.npy", "wb")
        np.save(filehandler, self.uCurrent)
        filehandler = open("bond-based-2d-plate-point-"+str(self.h)+"-"+str(self.delta_factor)+"-"+str(step)+"-damage.npy", "wb")
        np.save(filehandler,self.d)
        filehandler = open("bond-based-2d-plate-point-"+str(self.h)+"-"+str(self.delta_factor)+"-"+str(step)+"-b.npy", "wb")
        np.save(filehandler,self.b)
        filehandler = open("bond-based-2d-plate-point-"+str(self.h)+"-"+str(self.delta_factor)+"-"+str(step)+"-f.npy", "wb")
